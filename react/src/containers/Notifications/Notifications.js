...
class Notifications extends Component {
    ...
    loadData() {
        let user = JSON.parse(localStorage.getItem('user'));
         ...
         axios.get('http://localhost:4000/api/alerte/', {
            headers: {
    ...
    render() {
        ...
        let notifications = this.state.notifications.map(notification => {
            if (!notification.deleted) {
                if (notification.type !== 'stock perime' && notification.type !== 'stock epuise' ) {
                    count++;
                }
            }
            ...
            <React.Fragment>
                < div className="right Navigation-right" >
                    <div className='Notifications-wrapper'>
                        <p className={badgeClasses.join(' ')}>{count/2}</p>
                        <NotificationsIcon className="material-icons right Notification-material-icon" onClick={this.showNotificationsHandler}></NotificationsIcon>
